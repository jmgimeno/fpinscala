...
// In order to the tests for monads to compile and run I've needed to copy the
// answers of all the problems in the exercises package.
// I'll try to use as much as possible of the code we developed in class, but
// some of the "advanced" problems were never commented.
...
object Prop:
  ...
  // The Prop { ... } is an invocation to Prop.apply which "adaps" a Prop
  // defined with a function which does not have MaxSize into account to
  // one it does (but never uses it).
  ...
  // This is the version for SIZED generators in which the Prop has into account
  ...
  // This is de adapter for Props defined with a function which do not have size into account
  def apply(f: (TestCases, RNG) => Result): Prop =
    (_, n, rng) => f(n, rng)

  extension (self: Prop)

    // Copied from GenSuite
    def check(
             maxSize: MaxSize = 100,
