...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I,O] {
    ...
    def |>[O2](p2: Process[O,O2]): Process[I,O2] = p2 match {
      ...
      case Await(recv2) => this match {
        ...
        case Await(recv) => Await(oi => recv(oi) |> p2)
      }
    }
    /*
  ...
  object Process {
    ...
    def mean: Process[Double,Double] = {
      def go(n: Int, s: Double): Process[Double, Double] =
        await((d: Double) => {
          val newSum = s + d
          val newCount = n + 1
          emit(newSum/newCount, go(newCount, newSum))
        })
      go(0, 0.0)
    }
    ...
    def runLog(implicit F: MonadCatch[F]): F[IndexedSeq[O]] = {
      def go(cur: Process[F,O], acc: IndexedSeq[O]): F[IndexedSeq[O]] =
        cur match {
          ...
          case Await(req, recv) =>
            F.flatMap(F.attempt(req))(fe => go(Try(recv(fe)), acc))
        }
      go(this, IndexedSeq())
    }
